-- Comprehensive Database Cleanup and Fresh Data Seeding
-- This migration cleans all existing data and creates fresh sample data

-- STEP 1: Clean existing data (in correct order to avoid foreign key violations)
DELETE FROM schedule_assignments;
DELETE FROM time_entries;
DELETE FROM vacation_requests;
DELETE FROM employee_groups;
-- Update groups to remove manager references before deleting employees
...
-- Note: We don't delete from companies, user_profiles, or auth.users as they're managed by auth

-- STEP 2: Reset company data (keep existing companies but clean their data)
UPDATE companies SET 
  name = CASE 
    WHEN company_code = 'DEMO2025' THEN 'Demo Corporation GmbH'
    ELSE name
  END,
  slug = CASE 
    WHEN company_code = 'DEMO2025' THEN 'demo-corp'
    ELSE slug
  END,
  domain = CASE 
    WHEN company_code = 'DEMO2025' THEN 'demo.company'
    ELSE domain
  END,
  address = CASE 
    WHEN company_code = 'DEMO2025' THEN 'Musterstraße 123, 12345 Berlin, Deutschland'
    ELSE address
  END,
  phone = CASE 
    WHEN company_code = 'DEMO2025' THEN '+49 30 12345678'
    ELSE phone
  END,
  email = CASE 
    WHEN company_code = 'DEMO2025' THEN 'info@demo.company'
    ELSE email
  END,
  timezone = 'Europe/Berlin',
  settings = '{"work_hours": {"default": 8, "flexible": true}, "overtime_policy": {"max_daily": 4, "auto_approve_time_off": true}}'::jsonb
WHERE company_code = 'DEMO2025';

-- STEP 3: Create sample employees for the demo company
...
-- STEP 4: Create groups for the demo company
INSERT INTO groups (
...
-- STEP 5: Assign employees to groups
...
-- STEP 6: Create time slots for the demo company
...
-- STEP 7: Create sample schedule assignments for the current week
...
SELECT 
  ...
  'scheduled',
  (SELECT id FROM employees WHERE company_id = c.id AND employee_id = 'EMP003') -- Thomas Weber as creator
FROM companies c
JOIN employees e ON e.company_id = c.id
...
-- STEP 8: Create sample time entries for the past week
...
-- STEP 9: Create sample vacation requests
...
SELECT 
  ...
  request.reason,
  request.status,
  (SELECT id FROM employees WHERE company_id = c.id AND employee_id = 'EMP003'), -- Thomas Weber as approver
...
-- STEP 10: Update employee work hours in settings
UPDATE employees SET 
  hourly_rate = CASE employee_id
    WHEN 'EMP001' THEN 25.50
    WHEN 'EMP002' THEN 26.00
    WHEN 'EMP003' THEN 35.00
    WHEN 'EMP004' THEN 12.00
    WHEN 'EMP005' THEN 22.00
    WHEN 'EMP006' THEN 28.00
    WHEN 'EMP007' THEN 24.00
    WHEN 'EMP008' THEN 25.00
    ELSE hourly_rate
  END
WHERE company_id IN (SELECT id FROM companies WHERE company_code = 'DEMO2025');

-- Create a summary view of the created data
CREATE OR REPLACE VIEW demo_data_summary AS
SELECT 
  'Companies' as table_name,
  COUNT(*)::text as count,
  'Demo Corporation GmbH' as sample_data
FROM companies WHERE company_code = 'DEMO2025'
UNION ALL
SELECT 
  'Employees' as table_name,
  COUNT(*)::text as count,
  STRING_AGG(e.first_name || ' ' || e.last_name, ', ' ORDER BY e.employee_id) as sample_data
FROM employees e
...
SELECT 
  ...
  COUNT(*)::text as count,
  STRING_AGG(g.name, ', ' ORDER BY g.name) as sample_data
FROM groups g
...
SELECT 
  ...
  COUNT(*)::text as count,
  STRING_AGG(ts.name, ', ' ORDER BY ts.start_time) as sample_data
FROM time_slots ts
JOIN companies c ON ts.company_id = c.id
WHERE c.company_code = 'DEMO2025'
UNION ALL
SELECT 
  'Schedule Assignments' as table_name,
  COUNT(*)::text as count,
  'Current week assignments' as sample_data
...
SELECT 
  ...
  COUNT(*)::text as count,
  'Past week entries' as sample_data
...
UNION ALL
SELECT 
  'Vacation Requests' as table_name,
  COUNT(*)::text as count,
  'Mixed status requests' as sample_data
