...
-- STEP 1: Clean all existing data (CAREFUL - this removes ALL data!)
BEGIN;

-- Disable RLS temporarily for complete cleanup
ALTER TABLE IF EXISTS schedule_assignments DISABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS time_entries DISABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS vacation_requests DISABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS employee_groups DISABLE ROW LEVEL SECURITY;
...
-- Delete all data in correct order to avoid foreign key conflicts
DELETE FROM schedule_assignments;
DELETE FROM time_entries;
DELETE FROM vacation_requests;
DELETE FROM employee_groups;

-- Remove manager references from groups before deleting employees
UPDATE groups SET manager_id = NULL WHERE manager_id IS NOT NULL;

-- Delete all groups and time slots
DELETE FROM groups;
DELETE FROM time_slots;

-- Only delete employees that are not linked to auth users
DELETE FROM employees WHERE auth_user_id IS NULL;

-- Re-enable RLS
ALTER TABLE IF EXISTS schedule_assignments ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS time_entries ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS vacation_requests ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS employee_groups ENABLE ROW LEVEL SECURITY;
...
-- ===================================================================
-- STEP 2: Create/Update Demo Company
-- ===================================================================
...
-- ===================================================================
-- STEP 7: Create Sample Schedule Assignments for Current Week
-- ===================================================================
...
-- ===================================================================
-- STEP 8: Create Sample Time Entries
-- ===================================================================
...
SELECT 
  'Database Cleanup and Seeding Complete!' as status,
  (SELECT COUNT(*) FROM companies WHERE company_code = 'DEMO2025') as companies_count,
  (SELECT COUNT(*) FROM employees WHERE company_id IN (SELECT id FROM companies WHERE company_code = 'DEMO2025')) as employees_count,
  (SELECT COUNT(*) FROM groups WHERE company_id IN (SELECT id FROM companies WHERE company_code = 'DEMO2025')) as groups_count,
  (SELECT COUNT(*) FROM time_slots WHERE company_id IN (SELECT id FROM companies WHERE company_code = 'DEMO2025')) as time_slots_count,
  (SELECT COUNT(*) FROM schedule_assignments WHERE company_id IN (SELECT id FROM companies WHERE company_code = 'DEMO2025')) as schedule_assignments_count,
  (SELECT COUNT(*) FROM time_entries WHERE employee_id IN (SELECT id FROM employees WHERE company_id IN (SELECT id FROM companies WHERE company_code = 'DEMO2025'))) as time_entries_count,
  (SELECT COUNT(*) FROM vacation_requests WHERE employee_id IN (SELECT id FROM employees WHERE company_id IN (SELECT id FROM companies WHERE company_code = 'DEMO2025'))) as vacation_requests_count;

-- ===================================================================
-- DONE! Your database should now be clean and seeded with fresh demo data.
-- 
-- Next Steps:
-- 1. Create user accounts through registration with company code: DEMO2025
-- 2. All passwords: Demo123!
-- 3. Admin user: thomas.weber@demo.company
-- ===================================================================